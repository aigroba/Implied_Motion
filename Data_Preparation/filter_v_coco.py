...
def get_img_ids(subj: str, 
                NSD_PATH: Path = Path("path/to/Datasets/NSD/"))-> dict:
...
def get_mscoco(subj: str, NSD_PATH: Path = Path("path/to/Datasets/NSD/"))-> dict:

    stim_info_path = NSD_PATH / 'stimuli'/ 'nsd_stim_info_merged.csv'
    nsd_ids = get_img_ids(subj, NSD_PATH)
...
def get_vcoco_list(v_coco_path:Path = Path("path/to/v-coco/data/splits/vcoco_all.ids"))-> list:
    # load the v-coco Ids
...
def get_vcoco_nsd(subj: str, v_coco_path:Path = Path("path/to/v-coco/data/splits/vcoco_all.ids")):
...
def main():
    ...
    for subj in subj:
        ...
        save_file = save_path / "v_coco_info.csv"
        save_file_2 = save_path / "v_coco_list.csv"
